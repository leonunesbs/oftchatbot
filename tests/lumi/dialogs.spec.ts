...
const scenarios: DialogScenario[] = [
  ...
  },
  {
    id: "20-multi-entity",
    turns: [
      "quero agendar, sou Leo Nunes, meu telefone é 85988887777, fortaleza, consulta geral, quinta à tarde",
    ],
  ...
  },
  {
    ...
    expectedState: "SCHEDULING_COLLECT_PHONE",
    expectReplyIncludes: ["telefone"],
  },
];
  ...
  for (const scenario of scenarios) {
    ...
    if (scenario.expectedState) {
      assert.equal(
        ...
        `State mismatch in ${scenario.id}`,
      );
    }
    if (scenario.expectHandoff !== undefined) {
      assert.equal(
        result.handoff,
        scenario.expectHandoff,
        `Handoff mismatch in ${scenario.id}`,
      );
    }
    if (scenario.expectReplyIncludes) {
      for (const expectedText of scenario.expectReplyIncludes) {
        assert.ok(
          result.reply.includes(expectedText),
          `Reply in ${scenario.id} should include: ${expectedText}`,
        );
      }
    }
  }
