...
type CityRow = (typeof siteConfig.cities)[number];
...
export function usePreferredCityOrder() {
...
export interface BookingDialogPanelProps {
  triggerId?: string;
...
export function BookingDialogPanel({
  triggerId,
  onlineBookingLinkId,
...
}: BookingDialogPanelProps) {
  const orderedCities = usePreferredCityOrder();
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-3 pt-4">
        {orderedCities.map((city: CityRow) => (
          <a
            key={city.slug}
      ...
      {showOnlineBookingCta ? (
        <div className="mt-5 rounded-xl border border-border/70 bg-muted/30 p-4">
          <p className="text-sm text-muted-foreground">
            Prefere escolher data e horário agora? Faça seu agendamento online em poucos
            cliques.
          </p>
